module MulDiv (
	...
	always @(posedge clock) begin : sv2v_autoblock_1
		reg [2:0] decoded_invInputs;
		reg [1:0] _decoded_andMatrixOutputs_T;
		reg lhs_sign;
		reg rhs_sign;
		reg _GEN;
		reg _GEN_0;
		...
		reg _GEN_4;
		reg _GEN_5;
		decoded_invInputs = ~io_req_bits_fn[2:0];
		_decoded_andMatrixOutputs_T = {decoded_invInputs[1], decoded_invInputs[2]};
		lhs_sign = |{decoded_invInputs[0], &_decoded_andMatrixOutputs_T} & (io_req_bits_dw ? io_req_bits_in1[63] : io_req_bits_in1[31]);
		rhs_sign = |{&_decoded_andMatrixOutputs_T, &{decoded_invInputs[0], io_req_bits_fn[2]}} & (io_req_bits_dw ? io_req_bits_in2[63] : io_req_bits_in2[31]);
		_GEN = state == 3'h1;
		_GEN_0 = state == 3'h5;
		_GEN_1 = state == 3'h2;
		...
		if (reset)
			...
			state <= 3'h3;
		if (_GEN_5) begin : sv2v_autoblock_2
			reg [1:0] _decoded_orMatrixOutputs_T_4;
			_decoded_orMatrixOutputs_T_4 = {&{io_req_bits_fn[0], decoded_invInputs[2]}, io_req_bits_fn[1]};
			req_dw <= io_req_bits_dw;
			count <= {1'h0, decoded_invInputs[2] & ~io_req_bits_dw, 5'h00};
			...
			else if (_GEN_1) begin : sv2v_autoblock_5
				reg [65:0] _prod_T_4;
				_prod_T_4 = ({{65 {remainder[64]}}, remainder[0]} * {divisor[64], divisor}) + {remainder[129], remainder[129:65]};
				count <= count + 7'h01;
